...
pickle_model = best_clf_lr.best_estimator_
# Productionizing model 
import pickle
pickl = {'model': pickle_model}
...
df_input = pd.DataFrame(X_test[1,:])
list(df_input.iloc[:,0])

#lr.fit(X_train_std, y_train)
#x = np.array(list(df_input.iloc[:,0])).reshape(1,-1)
